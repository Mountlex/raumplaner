...
impl From<RoleType> for Role {
    fn from(value: RoleType) -> Self {
        match value {
            ...
            RoleType::DefaultUser => Role::User,
        }
    }
}
...
pub async fn authorize(
    ...
    match user::Entity::find_by_id(payload.client_id)
        .one(&state.conn)
        .await
    {
        Ok(Some(user)) if payload.client_secret == user.password => {
            let claims = Claims {
                sub: user.user_name,
...
impl fmt::Display for AuthError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let msg = match *self {
            ...
            AuthError::TokenCreation => "Token creation error",
